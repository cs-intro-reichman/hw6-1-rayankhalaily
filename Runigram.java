...
public class Runigram {
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int i = 0; i < width; i++){
			for (int j = 0; j < height; j++){
				newImage[j][i] = image[height - 1 - j][i];
			}
		}
		return newImage;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
    	...
    	for (int i = 0; i < height; i++) {
        	for (int j = 0; j < width; j++) {
            	int sourceRow = (int)(i * (h0 / (double) height));
            	int sourceCol = (int)(j * (w0 / (double) width));
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
 	   ...
 	   // Scale target to match source dimensions
 	   target = scaled(target, width, height);

 	   for (int i = 0; i <= n; i++) {
   	  		double alpha = (n - i) / (double) n;   // starts at 1, ends at 0
